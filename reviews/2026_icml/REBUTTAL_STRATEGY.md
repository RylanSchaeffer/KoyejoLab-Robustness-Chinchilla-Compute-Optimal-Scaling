...
**What to do in the revised manuscript:**
- Revise "all four sensitivity analyses demonstrate that Chinchilla's key results withstand sizable perturbations" to something precise: "...withstand perturbations beyond the 15.2% discrepancy we identified, with the nature and threshold of breakdown depending on the perturbation type (see Sections 3.1–3.4)."
...
To our knowledge, no prior scaling law paper has performed this kind of sensitivity analysis. We are not just auditing Chinchilla — we are establishing that covariate sensitivity analysis should be standard practice for scaling law research, and providing the template.
...
- Reframe the Future Directions paragraph from "we could do this for other scaling laws" (a to-do item) to a concrete methodological call to action: every scaling law paper that derives prescriptions — Muennighoff et al. (data repetition), Gadre et al. (overtraining), Sardana et al. (inference-efficient scaling), MoE scaling, and others — should assess the sensitivity of their prescriptions to covariate specification errors, using the perturbation taxonomy and analytical toolkit we provide.
...
**What to do in the rebuttal text:**
- "We recommend the Standard Formula (Eqn. 1) for three reasons: (1) it is derived from architectural first principles with no free parameters, (2) it aligns with the convention of excluding certain parameters from N when computing $C \approx 6ND$, consistent with Kaplan et al., and (3) it produces the flattest D/N ratio with compute (slope = -0.572 per decade vs -1.049 and -1.248), strengthening the Chinchilla finding. We will also state the concrete D/N ratio under each formula to directly address the reviewer's interest in the 20-tokens-per-parameter rule."
...
**What to do in the rebuttal text:**
- "Those works each identified *specific* accounting decisions (head parameters, embeddings, optimizer settings) that explain discrepancies *between* Chinchilla and Kaplan. Our work is complementary but distinct: (1) we discover a previously unnoticed ambiguity *within* Chinchilla's own reported data, (2) we provide a *systematic* sensitivity analysis across four perturbation families rather than a single fix, and (3) we derive analytical explanations that yield a general diagnostic framework rather than case-by-case corrections. Indeed, our additive-constant analysis (Sec 3.2) provides a unifying mathematical framework that reproduces the quantitative shifts reported by both Porian et al. and Pearce & Song (as noted on manuscript page 6, where the quantitative agreement is shown)."

**Note:** For rRfE specifically, this differentiation should be lighter-touch — rRfE already endorses our originality and a heavy-handed response risks looking more worried about originality than the reviewer is.
...
- This is a distinct ask from "modern relevance" — rRfE wants concrete evidence that practitioners still rely on Chinchilla-style scaling. "Chinchilla's compute-optimal prescriptions directly informed the training of DeepSeek and Llama 3, and its tokens-per-parameter heuristic remains a standard reference point in scaling discussions. Even in overtrained regimes that deviate from the Chinchilla-optimal point, practitioners use the Chinchilla baseline to quantify *how much* they are overtraining. LLaMA 1 (Touvron et al., 2023) similarly adopted Chinchilla-style scaling as its starting point."
...
- "Overtraining scaling laws (Gadre et al., Schaeffer et al. 2025) define their results as deviations from the Chinchilla compute-optimal point. If that optimal point were wrong — e.g., because the parameter counts were misspecified — every overtraining result that references it would inherit that error. Our paper shows the compute-optimal point is robust to the specification errors we identified, which means the mathematical foundation that overtraining and inference-efficient scaling papers build on is solid. The diagnostic framework itself is also regime-independent: the analytical relationships between perturbation type and scaling parameter distortion hold for any power-law fit."

**rRfE: Trivial experimental conditions**
- "rRfE's summary describes the perturbation analysis as studying 'trivial experimental conditions.' We want to clarify that the four perturbation families were chosen to span the realistic space of covariate specification errors, and that the analytical results reveal non-trivial structure: the distinction between multiplicative errors (which preserve the scaling exponent) and additive errors (which distort it) was previously unknown and has concrete diagnostic implications."

**rRfE: What research does this paper enable**
- "rRfE asks for 'a more nuanced future work section [that] could help clarify the sort of research enabled by this paper.' We will add a concrete discussion: every scaling law paper that derives prescriptions — Muennighoff et al. (data repetition scaling), Gadre et al. (overtraining scaling), Sardana et al. (inference-efficient scaling), MoE scaling laws, and others — should similarly assess the sensitivity of their prescriptions to covariate specification errors, using the perturbation taxonomy and analytical toolkit we provide. Additionally, our diagnostic framework opens the door to extending sensitivity analysis to data-side covariates (e.g., token counts, data quality metrics) and to designing scaling experiments that are inherently robust to specification errors."

**rRfE: Reporting checklist for future scaling papers**
- "rRfE asks how future papers can avoid parameter-counting ambiguity. We will add a concrete recommendation: scaling law papers should (1) state the exact parameter-counting formula used, (2) specify whether embedding parameters are included in N, (3) specify whether tied weights are counted once or twice, and (4) report both the C ≈ 6ND approximation and the exact FLOP count."
